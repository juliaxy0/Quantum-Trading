...
class backtestClass:
    ...
    def start_backtest(self, start, strategy):
        ...
        try:
            ...
            merged_close = pd.concat([btc_close, eth_close, link_close, ltc_close], axis=1)
            merged_close.columns = ['BTC/USD', 'ETH/USD', 'LINK/USD', 'LTC/USD']

            # Call the corresponding strategy based on the value of strategy
            if strategy == "SMA":
                entries, exits = self.SMA(merged_close, fast=10, slow=20)
            elif strategy == "RSI":
                entries, exits = self.RSI(merged_close, rsi_window=14, entry=70, exit=30)
            elif strategy == "MACD":
                entries, exits = self.MACD(merged_close, volume_col='volume', entry_threshold=0, exit_threshold=0)
            elif strategy == "BB":
                entries, exits = self.BB(merged_close, window=14, entry_z_score=-1, exit_z_score=1)
            elif strategy == "MACD_RSI":
                entries, exits = self.MACD_RSI(merged_close, 14, 30, 70)
            else:
                # Handle the case when an invalid strategy is provided
                print(f"Invalid strategy: {strategy}")

            pf_kwargs = dict(size=np.inf, fees=0.001, freq='1D')
            pf = vbt.Portfolio.from_signals(merged_close, entries, exits, **pf_kwargs)

            total_return = pf.total_return()
            total_return_json = total_return.to_json()
            
            return total_return_json
